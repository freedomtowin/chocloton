...
import warnings
from chocloton import ChocloOptimizer
import sys
import copy
from functools import partial
from .hdre_helpers import *
...
class DensityFactorization():
    ...
    def update_kde(self,X,y=None,alpha=0.9):
    ...
    def optimize(self, X, y=None, cost=None,
                number_of_random_simulations=1000,
                number_of_realizations=10, agg_func='count',verbose=True,
                multi_dim=False,
        ):
    ...
    #             bins3 = [np.concatenate([[-np.inf],_bins_,[np.inf]]) for _bins_ in bins3]
        ...
        param_to_dim = param_to_dim.astype(np.float64)
        ...
        if self.kmldata is None:
            ...
            self.kml.load_choclo_data(self.kmldata)
#            self.kml.choclo_data.args = self.args
        ...
        self.kmldata = self.kml.choclo_data
        self.kml.load_choclo_data(self.kmldata)
...
#             self.kde_target[count] = np.flipud(self.kde_target[count])
    ...
    def get_kde_estimate(self, pad=1.0):
        ...
        for i,j in self.dim_combos:
            ...
            for k in range(self.num_clusters):
                ...
                pdf = uniform_kernel(bins3[0], mean[k,i], var[i,0]).dot(
                    uniform_kernel(bins3[1],mean[k,j],var[j,0]).T
                )
    ...
    def get_assignments(self,X,pad=1):
        var = self.deviations_
        mean = self.centroids_
        mask = np.zeros((X.shape[0],self.num_clusters),dtype=bool)
        count=0
        for k in range(self.num_clusters):
...
class HierarchicalDensityFactorization():
    ...
    def refactor_assignments(self,X,pad=1.):
        ...
        for model in self.models:
            ...
            D = model.get_assignments(X,pad=1.).astype(np.int64)
...
#        return np.hstack(assignments)
    ...
    def assign(self,X,pad=1):
        ...
        mask = np.zeros((X.shape[0],num_clusters),dtype=bool)
        k=0
...
#             self.KDE = self.model.kde_target.copy()
        ...
        D = self.model.get_assignments(X,pad=1.).astype(np.int64)
